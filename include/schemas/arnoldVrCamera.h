...
public:
    ...
    USDARNOLD_API
    UsdAttribute GetEyeSeparationAttr() const;

    /// See GetEyeSeparationAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    USDARNOLD_API
    UsdAttribute CreateEyeSeparationAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
...
public:
    ...
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Float |
    USDARNOLD_API
    UsdAttribute GetEyeToNeckAttr() const;

    /// See GetEyeToNeckAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    USDARNOLD_API
    UsdAttribute CreateEyeToNeckAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
...
public:
    ...
    USDARNOLD_API
    UsdAttribute GetTopMergeModeAttr() const;

    /// See GetTopMergeModeAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    USDARNOLD_API
    UsdAttribute CreateTopMergeModeAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
...
public:
    ...
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Float |
    USDARNOLD_API
    UsdAttribute GetTopMergeAngleAttr() const;

    /// See GetTopMergeAngleAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    USDARNOLD_API
    UsdAttribute CreateTopMergeAngleAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
...
public:
    ...
    /// | \ref UsdArnoldTokens "Allowed Values" | none, cosine, shader |
    USDARNOLD_API
    UsdAttribute GetBottomMergeModeAttr() const;

    /// See GetBottomMergeModeAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    USDARNOLD_API
    UsdAttribute CreateBottomMergeModeAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
...
public:
    ...
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Float |
    USDARNOLD_API
    UsdAttribute GetBottomMergeAngleAttr() const;

    /// See GetBottomMergeAngleAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    USDARNOLD_API
    UsdAttribute CreateBottomMergeAngleAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
...
public:
    ...
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Float |
    USDARNOLD_API
    UsdAttribute GetMergeShaderAttr() const;

    /// See GetMergeShaderAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    USDARNOLD_API
    UsdAttribute CreateMergeShaderAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
